-- SOLUÇÃO ULTRA SIMPLES - ACESSO DIRETO
...
BEGIN
  ...
  VALUES (admin_user_id, admin_email, crypt('admin123', gen_salt('bf')), NOW(), NOW(), NOW(), 
          jsonb_build_object('admin_access', true, 'role', 'establishment', 'establishment_id', establishment_id, 'establishment_name', establishment_record.name, 'establishment_code', establishment_record.code, 'admin_impersonation', true, 'full_name', establishment_record.name), 
          'authenticated', 'authenticated');

  RETURN jsonb_build_object('success', true, 'admin_user_id', admin_user_id, 'admin_email', admin_email, 'admin_password', 'admin123', 'establishment_id', establishment_record.id, 'establishment_name', establishment_record.name, 'establishment_code', establishment_record.code, 'original_email', establishment_record.email, 'message', 'Usuário admin criado com sucesso');
END;
