-- ACESSO DIRETO DO DONO - SEM SENHA
...
-- Função para criar usuário admin temporário com acesso total
...
BEGIN
  ...
  -- Gerar ID único para usuário admin
...
-- Função para limpar usuários admin antigos
CREATE OR REPLACE FUNCTION cleanup_admin_users()
RETURNS JSONB AS $$
BEGIN
  -- Remover usuários admin temporários antigos (mais de 1 hora)
  ...
  RETURN jsonb_build_object(
    'success', true,
    'message', 'Usuários admin antigos removidos'
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
GRANT EXECUTE ON FUNCTION cleanup_admin_users() TO anon;
GRANT EXECUTE ON FUNCTION cleanup_admin_users() TO authenticated;

-- Comentários
COMMENT ON FUNCTION create_admin_access(UUID) IS 'Cria usuário admin temporário para acesso direto do dono';
COMMENT ON FUNCTION cleanup_admin_users() IS 'Remove usuários admin temporários antigos';
