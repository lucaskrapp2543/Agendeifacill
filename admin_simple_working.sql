-- Sistema de Senha Administrativa - Versão Simples e Funcional
...
-- 2. Criar função simples que apenas verifica
CREATE OR REPLACE FUNCTION verify_admin_access(user_email TEXT, admin_password TEXT)
RETURNS JSONB AS $$
DECLARE
  user_record RECORD;
...
-- 3. Conceder permissões amplas
GRANT EXECUTE ON FUNCTION verify_admin_access(TEXT, TEXT) TO anon;
GRANT EXECUTE ON FUNCTION verify_admin_access(TEXT, TEXT) TO authenticated;

-- 4. Comentário
COMMENT ON FUNCTION verify_admin_access(TEXT, TEXT) IS 'Verifica acesso administrativo sem alterar senhas';
