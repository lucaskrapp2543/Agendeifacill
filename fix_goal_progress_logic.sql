-- Correção da função get_professional_goal_progress
-- O problema é que a lógica não está filtrando corretamente pelos serviços selecionados
    ...
    WHERE a.establishment_id = p_establishment_id
      ...
      AND (
        ...
        EXISTS (
          SELECT 1 
          FROM goal_data gd
          WHERE gd.selected_services @> to_jsonb(a.service)
          OR gd.selected_services @> to_jsonb(a.service::text)
          -- Também verificar se o serviço está nas subcategorias selecionadas
          OR EXISTS (
            SELECT 1 
            FROM service_subcategories ss
            WHERE ss.id::text = ANY(
              SELECT jsonb_array_elements_text(gd.selected_services)
            )
            AND ss.name = a.service
          )
        )
      )
  )
