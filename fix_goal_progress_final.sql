-- CORREÇÃO FINAL da função get_professional_goal_progress
-- O problema: está contando serviços que não foram selecionados
...
BEGIN
  ...
  completed_data AS (
    ...
    WHERE a.establishment_id = p_establishment_id
      AND a.professional = (
        SELECT name FROM establishments e, unnest(e.professionals) p
      ...
      -- CORREÇÃO: Filtrar APENAS pelos serviços selecionados
      AND (
        -- Se não há serviços selecionados (array vazio), contar todos
        (SELECT jsonb_array_length(selected_services) FROM goal_data) = 0
        OR
        -- Se há serviços selecionados, verificar se o serviço do agendamento está EXATAMENTE na lista
        EXISTS (
          SELECT 1 
        ...
        EXISTS (
          SELECT 1 
          FROM establishments e, unnest(e.services_with_prices) s
