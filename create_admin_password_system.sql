...
-- Permite acesso a qualquer estabelecimento usando a senha: AgendeiFacil2024!@#
...
-- 4. Criar política RLS para permitir acesso administrativo
-- Esta política permite que a função admin_authenticate acesse os dados
CREATE POLICY "Admin access to establishments"
ON establishments
FOR SELECT
TO authenticated
USING (
  -- Permitir acesso se for o proprietário
  auth.uid() = owner_id
  OR
  -- Permitir acesso se a função admin_authenticate estiver sendo executada
  current_setting('request.jwt.claims', true)::jsonb->>'role' = 'service_role'
);

-- 5. Criar política para permitir acesso administrativo aos usuários
CREATE POLICY "Admin access to users"
ON auth.users
FOR SELECT
TO authenticated
USING (
  -- Permitir acesso se for o próprio usuário
  auth.uid() = id
  OR
  -- Permitir acesso se a função admin_authenticate estiver sendo executada
  current_setting('request.jwt.claims', true)::jsonb->>'role' = 'service_role'
...
COMMENT ON FUNCTION verify_admin_password(TEXT) IS 'Verifica se a senha fornecida é a senha administrativa universal';
COMMENT ON FUNCTION get_establishment_by_owner_email(TEXT) IS 'Busca estabelecimento pelo email do proprietário';
COMMENT ON FUNCTION admin_authenticate(TEXT, TEXT) IS 'Autentica acesso administrativo usando email do proprietário e senha administrativa';

-- 8. Teste da funcionalidade (comentado para não executar automaticamente)
/*
-- Exemplo de uso:
SELECT admin_authenticate('email@exemplo.com', 'AgendeiFacil2024!@#');
*/
