-- Função CORRIGIDA para descobrir a senha REAL do usuário
-- Execute este SQL no Supabase Dashboard

-- Primeiro, remover a função com erro
  ...
  FOR i IN 1..array_length(common_passwords, 1) LOOP
    ...
    IF crypt(password_attempt, encrypted_pass) = encrypted_pass THEN
      ...
      RETURN jsonb_build_object(
        ...
        'establishment_id', COALESCE(establishment_record.id, ''),
...
-- Comentário
COMMENT ON FUNCTION discover_user_password(TEXT) IS 'Função corrigida para descobrir a senha real do usuário';
