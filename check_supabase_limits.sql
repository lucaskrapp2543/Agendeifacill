...
-- 1. TAMANHO ATUAL DO BANCO vs LIMITE
SELECT 
    ...
    '8 GB' as limite_pro,
    ROUND(
        (pg_database_size(current_database())::numeric / (500 * 1024 * 1024)) * 100, 2
    ) as percentual_free,
    ROUND(
        (pg_database_size(current_database())::numeric / (8 * 1024 * 1024 * 1024)) * 100, 2
    ) as percentual_pro;
