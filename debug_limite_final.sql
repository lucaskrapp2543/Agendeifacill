-- DEBUG FINAL DO LIMITE MENSAL
-- Execute este SQL para ver exatamente o que está acontecendo

-- 1. VERIFICAR SE O CLIENTE TEM ASSINATURA E LIMITE DEFINIDO
SELECT 
    '=== ASSINATURA COM LIMITE ===' as debug_info,
    cs.id,
    cs.client_whatsapp,
    cs.monthly_limit,
    cs.start_date,
...
-- 4. TESTE DA LÓGICA DE LIMITE
SELECT 
    '=== TESTE DA LÓGICA DE LIMITE ===' as debug_info,
    CASE 
        WHEN cs.monthly_limit IS NULL THEN 'SEM LIMITE'
        ELSE cs.monthly_limit::text || ' AGENDAMENTOS'
    ...
    CASE 
        WHEN cs.monthly_limit IS NULL THEN 'PODE AGENDAR (SEM LIMITE)'
        WHEN (
            SELECT COUNT(*) 
            FROM appointments 
            WHERE client_whatsapp = cs.client_whatsapp
            ...
            AND status IN ('confirmed', 'completed', 'pending')
            AND is_subscriber = true
        ) >= cs.monthly_limit THEN 'BLOQUEADO (LIMITE EXCEDIDO)'
