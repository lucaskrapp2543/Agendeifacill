-- CORRIGIR client_whatsapp copiando dos agendamentos
...
-- 3. ATUALIZAR client_whatsapp usando o WhatsApp dos agendamentos
UPDATE client_subscriptions 
SET client_whatsapp = (
    SELECT a.client_whatsapp 
    FROM appointments a 
    WHERE a.client_id = client_subscriptions.client_id::text
...
-- 5. Mostrar client_subscriptions com limite definido e WhatsApp preenchido
SELECT 
    cs.id,
    cs.client_id,
    cs.client_whatsapp,
...
-- 6. Testar se conseguimos encontrar o cliente com limite 1
SELECT 
    cs.id,
    cs.client_id,
    cs.client_whatsapp,
...
FROM client_subscriptions cs
LEFT JOIN appointments a ON a.client_id = cs.client_id::text 
    AND a.is_subscriber = true
