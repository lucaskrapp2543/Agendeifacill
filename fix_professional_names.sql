-- CORRIGIR NOMES DOS PROFISSIONAIS
...
-- Se o professional_id for igual ao product_id, está errado
UPDATE appointment_products 
SET professional_id = a.professional
FROM appointments a
WHERE appointment_products.appointment_id = a.id
AND appointment_products.professional_id = appointment_products.product_id;
...
SELECT 
  ...
  COUNT(CASE WHEN ap.professional_id IS NOT NULL AND ap.professional_id != ap.product_id THEN 1 END) as com_nome_correto,
  COUNT(CASE WHEN ap.professional_id = ap.product_id THEN 1 END) as com_id_errado,
