-- Incluir service_duration da assinatura no retorno da RPC para o booking usar a duração correta (ex.: 1h30) no resumo e no agendamento.
-- Sem isso, o fluxo "Agendar como Assinante" usava sempre 30 min.
...
COMMENT ON FUNCTION public.check_subscriber_by_whatsapp(TEXT, TEXT)
  IS 'Verifica se um WhatsApp é assinante ativo ou vencido. Retorna service_duration da assinatura para o booking usar no resumo (ex.: 1h30).';
