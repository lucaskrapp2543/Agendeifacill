...
BEGIN
  IF jsonb_typeof(COALESCE(to_jsonb(NEW.professionals), '[]'::jsonb)) <> 'array' THEN
    RAISE EXCEPTION 'professionals deve ser um array JSON';
  END IF;

  IF jsonb_typeof(COALESCE(to_jsonb(NEW.professionals_pins), '[]'::jsonb)) <> 'array' THEN
  ...
  FOR professional_item IN
    SELECT value
    FROM jsonb_array_elements(COALESCE(to_jsonb(NEW.professionals), '[]'::jsonb))
  LOOP
  ...
  IF jsonb_array_length(COALESCE(to_jsonb(NEW.professionals), '[]'::jsonb)) > 0 AND NOT has_owner THEN
  ...
  LOOP
    ...
    FOR pin_item IN
      SELECT value
      FROM jsonb_array_elements(COALESCE(to_jsonb(NEW.professionals_pins), '[]'::jsonb))
    LOOP
