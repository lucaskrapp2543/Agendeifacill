...
-- Resolve o "Erro ao registrar sessão" e o contador Suporte X/5 no painel admin.
-- =============================================================================
...
-- Função que retorna o e-mail do usuário logado (SECURITY DEFINER = consegue ler auth.users)
...
-- Remover policy antiga se existir (evita duplicata)
DROP POLICY IF EXISTS "support_sessions_only_support_email" ON public.support_sessions;

-- Apenas a conta de suporte pode ver/inserir/atualizar/deletar (usa a função para o RLS enxergar o email)
...
-- 2) RPC: registrar sessão ou dar heartbeat. Limite de 5 sessões (só conta as ativas).
...
COMMENT ON FUNCTION public.register_support_session(TEXT) IS
  'Registra ou atualiza heartbeat da sessão do suporte. Máx. 5 sessões.';
