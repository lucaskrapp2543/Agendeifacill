...
export const TopMonthlyWinnerCard: React.FC<TopMonthlyWinnerCardProps> = ({ winner, className = '', onDismiss }) => {
  const content = (
    <div
      className={`relative w-full rounded-2xl border border-white/10 bg-[#10131b] p-3 sm:p-5 ${className}`}
    >
      {onDismiss && (
        <button
          type="button"
          ...
          className="absolute top-2 right-2 inline-flex items-center justify-center w-7 h-7 rounded-md border border-white/15 bg-white/5 hover:bg-white/10 text-white/85 transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
      )}
        ...
        <div className="relative shrink-0">
          {winner.imageUrl ? (
            <img
              ...
              className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl object-cover border border-white/15"
            />
          ) : (
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl border border-white/15 bg-white/5 flex items-center justify-center">
              <Crown className="w-7 h-7 sm:w-8 sm:h-8 text-yellow-300" />
            </div>
          )}
          <div className="absolute -top-2 -right-2 px-2 py-0.5 rounded-full bg-yellow-400 text-black text-[10px] font-black uppercase tracking-wide">
            Top 1
          </div>
        </div>

        <div className="min-w-0 flex-1">
          <p className="text-[11px] sm:text-xs uppercase tracking-[0.15em] text-yellow-200/90 font-bold">
            Top 1 Agendei Fácil
          </p>
          ...
          <p className="mt-1 text-[13px] leading-4 sm:text-sm text-white/80 font-semibold">
