...
interface PixPaymentFormProps {
  ...
  onPixProofUpload: (url: string) => void;
...
export const PixPaymentForm = ({
  ...
  const copyPixKey = async () => {
    if (establishment.pix_key) {
      await navigator.clipboard.writeText(establishment.pix_key);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // e então chamar onPixProofUpload com a URL retornada
    
    // Por enquanto, vamos apenas simular com uma URL local
    const reader = new FileReader();
    reader.onload = (e) => {
      const url = e.target?.result as string;
      onPixProofUpload(url);
    };
          ...
          {!establishment.pix_key ? (
            ...
            <>
              <div className="space-y-2">
                <div className="font-medium text-gray-700">Chave PIX ({establishment.pix_key_type}):</div>
                <div className="flex gap-2 items-center">
                  <input
                    type="text"
                    readOnly
                    value={establishment.pix_key}
                    className="flex-1 px-4 py-2 bg-white text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                  />
                  <button
                    onClick={copyPixKey}
              ...
              <div className="space-y-2">
                <div className="font-medium text-gray-700">Envie o comprovante:</div>
                <label className="flex flex-col items-center justify-center w-full h-32 bg-white border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    <svg className="w-8 h-8 mb-4 text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 16">
                      <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2"/>
                    </svg>
                    <p className="mb-2 text-sm text-gray-500">Clique para enviar o comprovante</p>
