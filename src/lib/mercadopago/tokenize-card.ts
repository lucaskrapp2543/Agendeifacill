...
export interface TokenizeCardResult {
  token: string;
  first_six_digits: string; // BIN do cartão (obrigatório para buscar payment_method_id e issuer_id via backend)
  // ⚠️ payment_method_id e issuer_id NÃO são retornados aqui
  // Devem ser buscados via backend (Netlify Function) usando o BIN
}
...
export async function tokenizeMercadoPagoCard(
  ...
  try {
    ...
    // ⚠️ REMOVIDO: Chamadas diretas para API do Mercado Pago (causavam CORS)
    // payment_method_id e issuer_id devem ser buscados via backend (Netlify Function)
    // usando o BIN retornado aqui
    ...
    console.log('✅ [MP Tokenize] Token criado:', {
      ...
      note: 'payment_method_id e issuer_id devem ser buscados via backend usando o BIN',
    });
